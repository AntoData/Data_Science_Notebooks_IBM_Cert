...
and Venus_Magellan_MeterScaleSlope_Global_4641m.tif to train a HDBSCAN
...
to the same area. We will train the HDBSCAN model using a sample of the
data and then use those classified points as seeds for a KNeighbours
classifier to complete the classification (as HDBSCAN requires more 
resources)
...
k_range: [int] = [k / 10000 for k in range(8680, 8690, 1)]
...
for k in k_range:
    ...
    hdbscan_measure: HDBSCAN = HDBSCAN(
    ...
    hdbscan_measure.fit(sample_var_x)
    print("Number of clusters: ")
    n_clusters: int = len(np.unique(hdbscan_measure.labels_))
    number_clusters_array.append(n_clusters)
    if -1 in hdbscan_measure.labels_:
...
"""
The HDBSCAN sweep over k·eps_m0 provides little evidence of intrinsic 
cluster structure in the data. Although the number of clusters changes 
initially, the algorithm rapidly collapses toward a two-cluster outcome 
(one core group plus noise). This is reflected in all diagnostic curves.

Silhouette scores are consistently negative across the entire k range, 
indicating that points are more similar to other clusters than to their 
own assigned grouping. In practice, this means that whatever partition 
HDBSCAN produces does not correspond to well-separated structure in the 
feature space. The silhouette curve rises slightly for larger k values, 
but the improvements are marginal and never reach even weakly positive 
territory — a strong signal that the resulting clusters are not 
meaningful.

The Davies–Bouldin Index exhibits the same behaviour: a sharp drop from 
its initial extremely poor value, followed by a nearly flat response as 
k increases. This asymptotic region corresponds to the stage where 
HDBSCAN has already merged most points into two broad clusters and is no 
longer able to discover finer internal structure. Because the DBI change 
after the first sample point is minimal, any additional adjustment of k 
does not improve separation or compactness.

Taken together, these metrics demonstrate that the dataset lacks 
well-defined density-driven partitions — cluster quality is poor, and 
the rapid collapse into only two stable groups suggests that HDBSCAN 
fails to discover any robust, high-contrast cluster organisation. As a 
result, although k values in the vicinity of the asymptotic region are 
numerically less pathological, the overall diagnostic pattern indicates 
that no meaningful clustering solution is supported by this model for 
this dataset.
"""
best_k = 0.869
print("8. Creating the HDBSCAN algorithm with eps=k*eps_m0 where k={0}".format(
    best_k))
# Apply K-Means
hdbscan_mod: HDBSCAN = HDBSCAN(
    cluster_selection_epsilon=float(eps0_m * best_k),
    min_samples=50,
    min_cluster_size=50,
    metric='euclidean',
    cluster_selection_method="eom",
...
hdbscan_mod.fit(sample_var_x)
print("9. Getting classification points")
...
cluster_map = np.ma.masked_equal(cluster_map, -1)
print("12. Getting emissivity ranges")
...
out_png = r".\HDBSCAN_Venus_emissivity_roughness.png"
...
silhouette_avg_: float = silhouette_score(sample_var_x, hdbscan_mod.labels_)
