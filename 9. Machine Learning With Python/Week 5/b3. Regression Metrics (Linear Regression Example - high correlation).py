...
df_y: pd.DataFrame = original_df_y.groupby(by=["year",
                                               "month"])["anom"].mean()
df_x: pd.DataFrame = original_df_x.groupby(by=["year",
                                               "month"])["average"].mean()
...
"""
Our model performs OK. Look at R2 score:
0.73 is still low to trust the model, but it is close to be just good 
enough
"""