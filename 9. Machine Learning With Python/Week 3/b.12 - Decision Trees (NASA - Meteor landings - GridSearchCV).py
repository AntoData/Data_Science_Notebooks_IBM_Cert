...
grid_search_dec_tree_meteor: GridSearchCV = GridSearchCV(decision_tree_meteor,
                                                         scoring="accuracy",
                                                         param_grid=max_depths,
                                                         cv=5)
# 6. We train the model
grid_search_dec_tree_meteor.fit(df_x_scl, df_y)

# 7. Now we get the best model
best_max_depth: int = grid_search_dec_tree_meteor.best_params_["max_depth"]
max_accuracy_score: float = grid_search_dec_tree_meteor.best_score_
print("Best model has a max depth = {0}".format(best_max_depth))
print("With accuracy score = {0}".format(max_accuracy_score))

# 8. Let's build now the decision tree with the best max depth
...
decision_tree_meteor_best.fit(df_x_scl, df_y)

# 10. Let's plot the decision tree
plot_tree(decision_tree_meteor_best)
plt.show()

# 11. Let's get the thresholds for each node in the tree
