...
print("5. Building and training the model for different ks")
scores_by_k: dict = {}
deep_k: int = 200
acc = np.zeros(deep_k)
...
for k in range(1, deep_k + 1):
    ...
    print("5.1 Training the mode")
    knns_mg.fit(x_train, y_train)
    print("5.2 Predicting using the model")
    y_pred = knns_mg.predict(x_test)
    print("5.3 Getting accuracy score")
    acc_sc: float = accuracy_score(y_test, y_pred)
    print("Score = {0}".format(acc_sc))
    scores_by_k[k] = acc_sc
    acc[k - 1] = acc_sc
    std_acc[k - 1] = np.std(y_pred == y_test) / np.sqrt(y_pred.shape[0])

print("Best k")
best_k = max(scores_by_k.items(), key=operator.itemgetter(1))[0]
print(best_k)
print("Score: {0}".format(scores_by_k[best_k]))
print("")
