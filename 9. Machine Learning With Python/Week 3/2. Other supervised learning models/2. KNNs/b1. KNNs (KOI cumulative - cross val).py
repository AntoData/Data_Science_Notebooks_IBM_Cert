...
print("4. We create and train the KNN using training sets and different ks")
k_scores: dict = {}
k_max_scores: dict = {}
deep_k: int = 200
acc = np.zeros(deep_k)
for k in range(1, deep_k+1):
    ...
    knn_mod: KNeighborsClassifier = KNeighborsClassifier(n_neighbors=k)
    print("4.1 We apply cross val score with scoring accuracy")
    scores: [float] = cross_val_score(knn_mod, x_var_scaled, y_var,
                                      cv=4, scoring="accuracy")
    print("Scores = {0}".format(scores))
    print("4.2 We use cross val predict")
    y_pred = cross_val_predict(knn_mod, x_var_scaled, y_var, cv=4)

    k_scores[k] = scores
    k_max_scores[k] = max(scores)
    print("")
    acc[k - 1] = max(scores)

print("Best k")
best_k = max(k_max_scores.items(), key=operator.itemgetter(1))[0]
print(best_k)
print("Scores: {0}".format(k_scores[best_k]))
print("")
...
knn_best: KNeighborsClassifier = KNeighborsClassifier(n_neighbors=best_k)
