...
print("4. We create the params for our GridSearchCV")
k_param: [int] = [x for x in range(1, 100)]
params: dict = {"n_neighbors": k_param}
print("5. We create our KNN Classifier object")
knns_koi: KNeighborsClassifier = KNeighborsClassifier()
print("6. We create our GridSearchCV model with scoring accuracy")
grid_knns_koi: GridSearchCV = GridSearchCV(
    knns_koi, param_grid=params, cv=5, scoring="accuracy")
print("7. We train the model")
grid_knns_koi.fit(x_var_scaled, y_var)
print("8. We get the best k and the best score")
best_param: dict = grid_knns_koi.best_params_
best_score: dict = grid_knns_koi.best_score_
print("Best k = {0}".format(best_param))
print("Best score = {0}".format(best_score))

print("9. Let's predict now the object that were labeled as CANDIDATE "
      "using best k")
...
knn_best: KNeighborsClassifier = KNeighborsClassifier(
    n_neighbors=best_param["n_neighbors"])
knn_best.fit(x_var_scaled, y_var)
y_pred_cand = knn_best.predict(x_cand_scaled)
