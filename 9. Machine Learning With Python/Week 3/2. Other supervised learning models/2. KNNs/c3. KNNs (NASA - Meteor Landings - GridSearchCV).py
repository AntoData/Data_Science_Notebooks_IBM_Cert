...
print("4. Creating params parameter for k")
params: dict = {"n_neighbors": [x for x in range(1,201)]}

print("5. Building the KNN classifier model")
knns_meteor: KNeighborsClassifier = KNeighborsClassifier()

print("6. Creating GridSearchCV object with scoring=accuracy")
grid_meteor: GridSearchCV = GridSearchCV(knns_meteor, param_grid=params,
                                         cv=5, scoring="accuracy")
print("7. Training GridSearchCV model")
grid_meteor.fit(df_x_std, df_y)

print("8. Getting best score and best k")
best_score: float = grid_meteor.best_score_
best_k: dict = grid_meteor.best_params_
