...
print("5. We create and train the KNN using training sets and different ks")
k_scores: dict = {}
deep_k: int = 200
acc = np.zeros(deep_k)
...
for k in range(1, deep_k+1):
    ...
    print("7. We use the predicted values to get accuracy score")
    score: float = accuracy_score(y_test, y_pred)
    print("Model's score = {0}".format(score))
    k_scores[k] = score
    print("")
    acc[k - 1] = score
...
print("Best k")
best_k = max(k_scores.items(), key=operator.itemgetter(1))[0]
print(best_k)
print("Score: {0}".format(k_scores[best_k]))
print("")
