...
rf_meteor: RandomForestClassifier = RandomForestClassifier(n_estimators=100,
                                                           random_state=41)
print("5.a Getting scores with Cross Val Score")
rf_training_start: float = time.time()
acc_scores_rf = cross_val_score(rf_meteor, df_x_std, df_y, cv=5,
                             scoring="accuracy")
rf_training_end: float = time.time()
print("Accuracy scores Random Forest = {0}".format(acc_scores_rf))
print("Mean accuracy score Random Forest = {0}".format(acc_scores_rf.mean()))
...
y_pred_rf = cross_val_predict(rf_meteor, df_x_std, df_y, cv=5,
                              method="predict")
rf_prediction_end: float = time.time()
print("Cross val predict time = {0}".format(
    rf_prediction_end-rf_prediction_start))

print("4.b Building the XGBoost")
xgb_meteor: XGBClassifier = XGBClassifier(n_estimators=100,
                                          random_state=41)
print("5.b Cross val score")
classes_xgb: dict = {k_: v_ for v_, k_ in enumerate(df_y.unique())}
df_y_numeric = df_y.apply(lambda y: classes_xgb[y])
xgb_training_start: float = time.time()
acc_scores_xgb = cross_val_score(xgb_meteor, df_x_std, df_y_numeric, cv=5,
                                 scoring="accuracy")
xgb_training_end: float = time.time()
print("Accuracy scores XGBoost = {0}".format(acc_scores_xgb))
print("Mean accuracy scores XGBoost = {0}".format(acc_scores_xgb.mean()))
print("Cross val score time = {0}".format(xgb_training_end-xgb_training_start))
print("6.b Using cross val predict")
xgb_prediction_start: float = time.time()
y_pred_xgb = cross_val_predict(xgb_meteor, df_x_std, df_y_numeric, cv=5,
                               method="predict")
xgb_prediction_end: float = time.time()
print("Cross vall predict time = {0}".format(
    xgb_prediction_end-xgb_prediction_start))