...
print("5.a We create the Random Forest Classifier")
...
rt_end: float = time.time()

print("7.a We predict y using x test set")
y_pred_rf = rf_mod.predict(x_test)

print("8.a We use the predicted values to get accuracy score")
rf_score: float = accuracy_score(y_test, y_pred_rf)
print("Random Forest training time = {0}".format(rt_end - rf_start))
print("Random Forest's score = {0}".format(rf_score))

print("9.a Let's predict now the object that were labeled as CANDIDATE "
...
y_pred_cand = rf_mod.predict(x_cand_scaled)
x_cand["koi_disposition_y"] = y_pred_cand
print(x_cand)

print("5.b We create the XGBoost")
...
print("7.b We predict y using x test set")
y_pred_xgb = xgb_mod.predict(x_test)

print("8.b We use the predicted values to get accuracy score")
xgb_score: float = accuracy_score(y_test, y_pred_xgb)
print("XGBoost training time = {0}".format(xgb_end - xgb_start))
print("XGBoost's score = {0}".format(xgb_score))

print("9.b Let's predict now the object that were labeled as CANDIDATE "
      "using our random forest")
x_cand = df_koi_candidates[["koi_period",
                            "koi_duration",
                            "koi_depth", "koi_prad",
...
x_cand.dropna(inplace=True)
x_cand_scaled = scaler.fit_transform(x_cand)
y_pred_cand = xgb_mod.predict(x_cand_scaled)
x_cand["koi_disposition_y"] = y_pred_cand
print(x_cand)
