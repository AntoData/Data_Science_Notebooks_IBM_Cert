...
print("5. We pass the model as parameter to cross_val_score")
roc_auc_scs: [float] = cross_val_score(
    lin_svm, x_var_scaled, y_var, scoring="roc_auc", cv=5)
print("Roc Auc Scores = {0}".format(roc_auc_scs))

print("6. Using the model to predict using cross val predict with "
      "method decision_function")
y_pred = cross_val_predict(
    lin_svm, x_var_scaled, y_var, cv=5, method='decision_function')

print("7. We use the predicted values to get roc auc score")
score: float = roc_auc_score(y_var, y_pred)
