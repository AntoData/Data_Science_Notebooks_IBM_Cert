...
print("5. We build the object of type GridSearchCV")
param_grid_cv: dict = {"C": [10**x for x in range(-1, 5)]}
grid_cab_svm: GridSearchCV = GridSearchCV(lin_svm, param_grid=param_grid_cv,
                                          scoring="accuracy", cv=5)
print("6. We train the model")
grid_cab_svm.fit(df_x_std, df_y)

print("7. Getting the best model: Best accuracy score and best C")
best_acc_score: float = grid_cab_svm.best_score_
print("Best score = {0}".format(best_acc_score))
best_c: dict = grid_cab_svm.best_params_
print("Best estimator = {0}".format(best_c))
