...
# 6. Let's create the object for our decision tree whose criterion is
...
for max_depth_ in range(4, 20):
    ...
    # 7. We train the model using our training sets for variables x and
    # y
    decision_tree_stars.fit(x_train, y_train)

    # 8. We make some predictions using this model and our testing set
    # for variable x
    y_pred: pd.DataFrame = decision_tree_stars.predict(x_test)

    # 9. We use the our testing set for variable y and our predictions
    # to get this max depth's level accuracy score
...
# 11. Let's build again the decision tree with the best model
decision_tree_stars_best: DecisionTreeClassifier = DecisionTreeClassifier(
    criterion="entropy", max_depth=best_max_depth
)

# 12. Let's train the model again
decision_tree_stars_best.fit(x_train, y_train)

# 13. Let's plot the decision tree
plot_tree(decision_tree_stars_best)
plt.show()

# 14. Let's get the threshold for the different features in the nodes
# of the tree
