...
# 4. We build the decision tree with criterion entropy and iterate
# through max depth levels
best_max_depth: int = 0
max_accuracy_scores: np.array = np.array([0.0])
...
for max_depth_ in range(4, 21):
    ...
    # 5. We use cross_val_score to get the accuracy scores
    acc_scores: [float] = cross_val_score(decision_tree_earthquakes, df_scl,
                                          df_y, scoring="accuracy", cv=5)
    print("Max depth = {0} - Accuracy scores = {1}".format(max_depth_,
                                                           acc_scores))
    if acc_scores.mean() > max_accuracy_scores.mean():
        max_accuracy_scores = acc_scores
