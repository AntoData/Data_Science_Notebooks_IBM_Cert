...
print("5. We display a plot of inertia for different ranges of k "
      "to apply the Elbow method")
# Elbow method
k_range: [int] = [k for k in range(2, 14)]
inertia: [float] = []
for k in k_range:
...
plt.show()
print("5.1 It looks like k: [3-4] is the best choice")

print("6. Applying the standard scaler")
std_sclr: StandardScaler = StandardScaler()
df_x_scaled: pd.DataFrame = std_sclr.fit_transform(df_x)
k_range: [int] = [k for k in range(3, 7)]
for k in k_range:
    print("FOR K = {0}".format(k))
    print("7. Creating the Kmeans model")
    kmeans_model: KMeans = KMeans(init="k-means++", n_clusters=k, n_init=12)
    print("8. Training the model")
    kmeans_model.fit(df_x_scaled)
    print("9.1 Getting the labels for every bus stop")
    df_x.loc[:, "label"] = kmeans_model.labels_
    print("9.2 Getting cluster centers")
    ...
    colors = ['#{0}'.format(str(hex(rd.randint(0, 4294967296))).
        ...
        kmeans_model.n_clusters+1)]
    print("10.1 Adding each point in the dataset to the world map using "
    ...
    print("10.2 Adding centroids to the map")
    descaled_centroids = std_sclr.inverse_transform(centroids)
    # Add centroids with markers
    for i, row in enumerate(descaled_centroids):
        folium.Marker(
    ...
    # Display map
    m.save("kmeans_seville_bus_stops_grous_{0}.html".format(k))
    print("")
