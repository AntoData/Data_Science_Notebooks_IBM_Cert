...
Centers and Gov Health Clinics using DBSCAN algorith
"""
...
k_range: [int] = [k for k in range(95, 97)]
for k in k_range:
    ...
    dbscan_mod.fit(df_x[["latitude_scaled", "longitude_scaled",
                         "n_libraries_scaled", "n_health_centers_scaled",
                         "n_social_serv_scaled"]])
    print("7.4 Getting labels for each point")
    df_x["label"] = dbscan_mod.labels_
    ...
    # Display map
    m.save("dbscan_seville_bus_stops_grous_{0}.html".format(k))
    print("")
