...
train a Kmeans  algorithm and classify the different areas of the Pluto
 ...
 As both refer to the same area. We will sample our DEM data and use 
 that sample to train a HDBSCAN algorith. We will use the result of that
 algorithm to train a KNeighborsClassifier model and we will use this 
 one to predict clusters for the whole image. As doing this in one 
 step will blow up our RAM, we will apply a Divide and Conquer strategy
 and predict using KNeighborsClassifier in several iterations
"""
...
if np.any(pos):
    ...
    if vals.size >= 2:
        ...
        if nz.size:
            ...
            raise ValueError("DEM values are all identical; HDBSCAN "
                             "cannot form clusters.")
    else:
        raise ValueError("DEM values are all identical; HDBSCAN cannot "
                         "form clusters.")
print("eps_m = {0}".format(eps_m))

print("9. Building the kmeans model")
hdbscan_mod: HDBSCAN = HDBSCAN(cluster_selection_epsilon=eps_m, min_samples=50,
...
# Set custom tick labels
out_png = r".\HDBSCAN_pluto_elevation.png"
...
"""
CONCLUSION: HDBSCAN does not work particularly well with 1-D continuous
variables. With millions of points packed into a continuous band of 
values, HDBSCAN sees one huge, contiguous “dense” region ⇒ one giant 
cluster.
"""
