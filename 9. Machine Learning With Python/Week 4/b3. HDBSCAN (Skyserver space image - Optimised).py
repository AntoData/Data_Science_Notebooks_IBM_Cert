...
# The Gaussian blur removes slow variations so HDBSCAN sees compact
...
print("3.2 Getting epsilon 0 to apply later in HDBSCAN model")
...
print("Epsilon multiplier k = {0}, so we are applying epsilon = "
      "{1} * {0}".format(k, eps))
print("4. Creating the HDBSCAN algorithm with k={0}".format(k))
# Apply HDBSCAN
eps_m: float = float(eps * k)
print("Epsilon = {0}".format(eps_m))
print("4.1 Building the HDBSCAN object")
HDBSCAN_mod: HDBSCAN = HDBSCAN(min_samples=20,
                               metric="euclidean",
                               min_cluster_size=20,
                               cluster_selection_method="eom",
                               # merges → fewer gaps
                               clustder_selection_epsilon=eps_m,
                               prediction_data=True
                               )
print("4.2 Training the model")
HDBSCAN_mod.fit(pixels_x_keep)
print("4.3 Getting labels for each point")
labels_keep: np.ndarray = HDBSCAN_mod.labels_
...
print("5. Getting cluster centers")
...
print("6. Post-processing: Background will stay background (black)"
      " even if HDBSCAN accidentally assigned a label to a big dark region")
...
# noise/background. As HDBSCAN uses −1 for noise, using >=0 creates a
...
print("7. Plotting original + clusters with legend in one figure")
...
print("8. Plotting original image")
# Left: original image
ax1.imshow(image)
ax1.set_title("Original")
ax1.axis("off")
print("9. Plotting segmented image")
...
if n_clusters == 0:
    ...
    ax2.set_title("Clusters (HDBSCAN-style)")
    ax2.set_facecolor("black")
    ax2.axis("off")
    ...
    Image.fromarray(rgb8).save("skyserver_HDBSCAN.png")
...
plt.show()
"""
CONCLUSIONS: HDBSCAN consumes a lot of less RAM memory than DBSCAN. 
However, it took a lot of research and trial and error to get this 
combination of settings that provide a result good enough
"""
