...
a sample we can use to train a DBSCAN algorithm, as this algorithm is 
...
def sample_for_every_level_idx(
    ...
    for b in range(n_bins):
        ...
        if sel.size:
            ...
            n_samples: int = min(per_bin, sel.size)
...
print("2.1 Reprojecting dataset FeO to dataset NIR grid so the area matches")
# Reprojecting dataset FeO to dataset NIR grid so the area matches
...
print("2.2 Reprojecting NIR image to dataset FeO grid so the area matches")
...
eps_idx: np.ndarray = sample_for_every_level_idx(
    valid_feo_values, per_bin=400, n_bins=128, cap=min(50_000, n_points))
clust_idx: np.ndarray = sample_for_every_level_idx(
    valid_feo_values, per_bin=400, n_bins=128, cap=min(100_000, n_points))
...
if np.any(pos):
    ...
    if vals.size >= 2:
        ...
        if nz.size:
            ...
            raise ValueError("FeO values are all identical; DBSCAN "
                             "cannot form clusters.")
    else:
        raise ValueError("FeO values are all identical; DBSCAN cannot "
                         "form clusters.")
print("eps_m = {0}".format(eps_m))
print("6. Creating the DBSCAN algorithm with k={0}".format(k))
print("6.1 Building the DBSCAN object")
dbscan_mod: DBSCAN = DBSCAN(eps=eps_m, min_samples=10, metric='euclidean',
...
out_png = r".\DBSCAN_Moon_Lunar_Clementine.png"
