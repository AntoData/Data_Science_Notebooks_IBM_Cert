...
We will use image M20_JezeroCrater_CTXDEM_20m to train a DBSCAN 
...
dbscan_mod = OPTICS(
    min_samples=5,
    metric="euclidean",
    cluster_method="dbscan",  # <-- extract DBSCAN-style clusters at eps
    max_eps=eps_m,
    eps=eps0_m,
)
print("7.3 Training the model")
dbscan_mod.fit(x_valid)
...
labels[valid_mask] = dbscan_mod.labels_
...
plt.title('DBSCAN Classification Overlay on Image')
plt.axis('off')
plt.show()
